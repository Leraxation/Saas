...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({ data, editable: redisConfigured() });
}
...
function isValidPatch(body: unknown): body is DepartmentDataPatch {
  ...
  if (b.headcountFilled !== undefined) {
    if (typeof b.headcountFilled !== "number" || b.headcountFilled < 0 || b.headcountFilled > MAX_HEADCOUNT) {
      return false;
    }
  }
  if (b.headcountOpen !== undefined) {
    if (typeof b.headcountOpen !== "number" || b.headcountOpen < 0 || b.headcountOpen > MAX_HEADCOUNT) {
      return false;
    }
  }
