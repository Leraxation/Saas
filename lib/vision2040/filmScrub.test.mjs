...
function installFilmDom({
  ...
  let nextRafId = 1;
  const warnings = [];
  const calmQuery = { matches: false };
  const section = { getBoundingClientRect: () => ({ height: sectionHeight }) };
  ...
  const trigger = { offsetTop: 0, offsetHeight: triggerHeight };
  ...
  globalThis.fetch = async () => ({
    ok: true,
    async json() {
      return { count: manifestCount, pattern: "/vision2040/frames/f%04d.jpg" };
    },
  });
...
test("createFilmScrub settles on calm mode if reduced motion turns on before lazy GSAP resolves", async () => {
  const env = installFilmDom();
  ...
  await env.withWarnings(async () => {
    ...
    env.calmQuery.matches = true;
    await waitTurn();
    await waitTurn();
    await waitTurn();

    assert.equal(env.canvas.dataset.driver, "calm");
    assert.equal(env.canvas.dataset.frame, "0");
    assert.equal(env.rafs.size, 0);
...
test("createFilmScrub preserves the mapped frame when upgrading with held segments", async () => {
  const env = installFilmDom({
    sectionHeight: 1200,
    manifestCount: 11,
    scrollY: 600,
  ...
  await env.withWarnings(async () => {
    ...
    const handle = createFilmScrub({
      ...
      manifestUrl: "/vision2040/frames/manifest.json",
      segments: [
        { section: "overture", weight: 50, from: 0, to: 0.2 },
        { section: "overture", weight: 50, from: 0.2, to: 0.2 },
        { section: "overture", weight: 50, from: 0.4, to: 1 },
      ],
      loadDriver: () => driverPromise,
    });
    ...
    const [, nativeTick] = env.rafs.entries().next().value;
    nativeTick();
    await waitTurn();
    const frameBeforeUpgrade = env.canvas.dataset.frame;
    ...
    assert.equal(frameBeforeUpgrade, "2");
    assert.equal(env.canvas.dataset.driver, "gsap");
    assert.equal(env.canvas.dataset.frame, frameBeforeUpgrade);
