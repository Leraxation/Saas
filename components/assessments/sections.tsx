...
import { BandPill, Card, GapPill, StatTile } from "./ui";
import type { Report } from "@/lib/assessments/scoring";

/**
 * The report is split into sections so the dedicated Benchmarking, Development
 * Plan and Coaching pages and the full report all render the same components
...
const MODULE_LABELS: [keyof Report["completeness"], string][] = [
...
export function ParticipantHeader({ report }: { report: Report }) {
    ...
    <div className="bg-white rounded-xl border border-slate-200 p-6 flex flex-wrap items-center gap-8">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          {p.role} · {p.department} · {LEVEL_LABELS[report.level]}
        </p>
          ...
          {MODULE_LABELS.map(([key, label]) => (
            <span
              ...
              className={`rounded-full px-2.5 py-1 font-medium ${
                report.completeness[key] ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-400"
              }`}
            >
              {report.completeness[key] ? "✓" : "○"} {label}
            </span>
          ))}
        </div>
      </div>
...
export function CompetencySection({ report }: { report: Report }) {
  const radarSeries = [
    { label: "Self", color: "#6366f1", values: report.competencies.map((c) => c.self), fill: true },
...
export function HighlightsSection({ report }: { report: Report }) {
  return (
...
export function FeedbackResultsSection({ report, assessmentId }: { report: Report; assessmentId: string }) {
  return (
    <Card
    ...
    >
      {report.coverage.invited === 0 ? (
        <p className="text-sm text-slate-500">
          ...
          <Link href={`/assessments/${assessmentId}/feedback`} className="text-indigo-600 hover:text-indigo-800">
            Invite raters
          </Link>{" "}
...
export function BehavioralSection({ report, assessmentId }: { report: Report; assessmentId: string }) {
  return (
    <Card
    ...
    >
      {report.traits ? (
        ...
        <p className="text-sm text-slate-500">
          The behavioral assessment has not been completed.{" "}
...
export function CognitiveSection({ report, assessmentId }: { report: Report; assessmentId: string }) {
  return (
    <Card title="Cognitive assessment" subtitle="Reasoning accuracy against the norm group, with a per-domain view.">
      {report.cognitive ? (
        <>
          <div className="flex items-center gap-6">
            ...
            <div className="space-y-1.5">
              <p className="text-sm text-slate-700">
                <span className="font-semibold">{report.cognitive.percentile}th percentile</span> against the norm group
              </p>
        ...
        <p className="text-sm text-slate-500">
          The cognitive assessment has not been completed.{" "}
...
export function AlignmentSection({ report, assessmentId }: { report: Report; assessmentId: string }) {
  const measured = report.completeness.alignment;
  return (
    <Card
      title="Organizational alignment"
      subtitle={
        measured
          ? "Measured directly by the alignment assessment."
          : "Inferred from the competencies that carry each value — complete the alignment assessment to measure it directly."
      }
      action={
        measured ? null : (
          <Link
            href={`/assessments/${assessmentId}/run/alignment`}
      ...
      <div className="grid md:grid-cols-2 gap-x-8 gap-y-4">
        {report.values.map((v) => (
          <div key={v.valueId}>
            ...
            <ScoreBar value={v.score} color={measured ? "#8b5cf6" : "#c4b5fd"} />
            <p className="text-xs text-slate-500 mt-1.5">{v.statement}</p>
            {measured ? (
              <p className="text-[11px] text-slate-400 mt-0.5">
                Competency-based estimate for comparison: {v.derived.toFixed(0)}
              </p>
            ) : (
